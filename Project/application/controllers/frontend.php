    ...
    public function editproperty()
    {
        ...
        if($user->package_id > 0)
        {
            ...
            if(empty($listing_num) && $package->num_listing_limit ==0&& !$id)
            {
                ...
                redirect('frontend/myprofile/'.$this->data['lang_code'].'#content');
                exit();
	    ...
	    if($id)
        {
            ...
            if(sw_count($this->data['estate']) == 0)
            {
                ...
                redirect('frontend/myprofile/'.$this->data['lang_code'], 'refresh');
            }
...
//                                    echo "unset\r\n";
    ...
    public function ajax ($page_id)
    {
        ...
        echo json_encode(array('results'=>$this->data['results_json'], 'results_center'=>$results_center, 'print' => $output, 'order'=>$order, 'lang_id'=>$lang_id, 'total_rows'=>$config['total_rows'], 'query'=> $query)); 
        exit();
    }
